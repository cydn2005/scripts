...
// @version      2.7.7
// @description  屏蔽微博, 知乎, 小红书, B站含关键词的内容，修复闪现消失bug、极致性能优化、跨域同步
// @author       KasenRi
...
(function() {
    ...
    const BLOCK_IN_SEARCH_KEY = 'keyword_blocker_block_in_search';

    let cachedDeepSplit = GM_getValue(DEEP_SPLIT_KEY, 'false') === 'true';
    let cachedBlockInSearch = GM_getValue(BLOCK_IN_SEARCH_KEY, 'true') === 'true';
    ...
    function isZhihuProfilePage() {
        const host = window.location.hostname;
        const href = window.location.href;
        return host.includes('zhihu.com') && (href.includes('/people/') || href.includes('/org/'));
    }

    function isZhihuQuestionPage() {
        const host = window.location.hostname;
        const href = window.location.href;
        return host.includes('zhihu.com') && href.includes('/question/');
    }

    function isBiliHistoryPage() {
        const host = window.location.hostname;
        const href = window.location.href;
        return host.includes('bilibili.com') && href.includes('/history');
    }
    ...
    function isSearchPage() {
        ...
        if (host.includes('bilibili.com') && (href.includes('search.bilibili.com') || href.includes('/all') || href.includes('keyword='))) return true;
        if (host.includes('weibo.com') && (href.includes('s.weibo.com') || href.includes('/search'))) return true;
    ...
    const siteConfigs = {
        zhihu: {
            containerSelector: '.TopstoryItem, .Card.TopstoryItem, .ContentItem, .SearchItem',
    ...
    function initUIEvents() {
        ...
        list.addEventListener('keypress', (e) => {
            if (e.key === 'Enter' && e.target.classList.contains('kb-edit-input')) {
                const listItem = e.target.closest('.kb-edit-input').closest('.kb-list-item');
    ...
    function processContentElement(element, config) {
        if (!element || element.dataset.kbProcessed === 'true') return;

        if (isZhihuProfilePage() || isZhihuQuestionPage() || isBiliHistoryPage()) {
        ...
        // 核心防闪烁拦截：如果抓取到的文本太短（说明是刚挂载的骨架屏或空节点），直接跳过，等待下次渲染完整再判定
        ...
        const isMatched = BLOCK_KEYWORDS.some(k => {
            if (!k) return false;
            if (trimmedTitle.includes(k)) return true;
            if (cachedDeepSplit && k.length >= 2) {
                const prefix2 = k.substring(0, 2);
                if (trimmedTitle.includes(prefix2)) return true;
            }
            return false;
        });
        ...
        if (isMatched) {
            ...
            if (site === 'zhihu') {
                const parentItem = element.closest('.TopstoryItem, .Card.TopstoryItem, .ContentItem');
                if (parentItem) targetToRemove = parentItem;
            } else if (site === 'bilibili') {
                const feed = element.closest('.feed-card') || element.closest('.bili-feed-card');
                if (feed) targetToRemove = feed;
            } else if (site === 'xiaohongshu') {
                const note = element.closest('.note-item');
                if (note) targetToRemove = note;
            }

            targetToRemove.remove();
        } else {
            // 只有当内容完整渲染并确认不匹配后，才加锁标记为已处理
            element.dataset.kbProcessed = 'true';
        }
    }
    ...
    function processAllContent() {
        ...
        const elements = document.querySelectorAll(`${config.containerSelector}:not([data-kb-processed="true"])`);
    ...
    // 稍微延长防抖时间到 400ms，确保页面异步内容完全渲染落地后再做屏蔽检查
    const debouncedProcessAllContent = debounce(processAllContent, 400);
    ...
    function init() {
        ...
        const observer = new MutationObserver((mutations) => {
            ...
            if (hasNewNodes) {
                debouncedProcessAllContent();
            }
        });

        observer.observe(document.body, { childList: true, subtree: true });

        console.log(`四平台跨域同步屏蔽器 (防闪烁稳定版) 已启动: ${getCurrentSite()}`);
    }
