...
function isPDFJSPage() {
    return document.querySelector('div#viewer.pdfViewer, embed[type="application/pdf"], iframe[src*=".pdf"]') !== null;
}

function isValidWord(word) {
    if (!word || word.length < 3) return false;
    if (/\d/.test(word)) return false;  // skip pure numbers or mixed too short
    return true;  // Chinese words pass through
}
...
function cutSentence(word, offset, sentence, sentenceNum) {
    ...
    if (sentenceNum > 0) {
        ...
        let highlightedWord = '<b>' + word + '</b>';
        return arr.slice(start, end + 1).join('').replaceAll(word, highlightedWord);
    } else {
